...
OUTPUT_DIR = os.path.join(RESULTS_ROOT, "model_text")
os.makedirs(OUTPUT_DIR, exist_ok=True)

input_path = os.path.join(RESULTS_ROOT, "results.json")
output_json = os.path.join(OUTPUT_DIR, "model_text_matrix.json")
output_csv = os.path.join(OUTPUT_DIR, "model_text_matrix.csv")
...
for entry in results:
    ...
    if model not in models:
        models.append(model)
    if text_label not in texts:
        texts.append(text_label)
    cell_values.setdefault((model, text_label), []).append(score)

matrix = []
for model in models:
    row = []
    for text_label in texts:
        values = cell_values.get((model, text_label), [])
        row.append(sum(values) / len(values) if values else None)
    matrix.append(row)

# Sort models by average GeneralScore across texts
model_rows = []
...
print(f"Saved model/text matrix JSON to {output_json}")
print(f"Saved model/text matrix CSV to {output_csv}")

# Print simple table
col_widths = [max(len(str(item)), 7) for item in header]
for j, text_label in enumerate(texts, start=1):
    col_widths[j] = max(col_widths[j], len(text_label))
for model in models:
...
# Best and worst model per text
per_text = {}
for j, text_label in enumerate(texts):
...
# Spread per model across texts (max - min of row)
model_spread = []
for model, row in zip(models, matrix):
...
analysis = {
    "best_worst_per_text": per_text,
    "most_consistent_model": model_spread[0],
    "most_variable_model":   model_spread[-1],
    "spread_ranking": model_spread,
}

analysis_path = os.path.join(OUTPUT_DIR, "model_text_analysis.json")
with open(analysis_path, "w") as f:
    json.dump(analysis, f, indent=2, ensure_ascii=False)
print(f"\nSaved analysis to {analysis_path}")

print("\n--- Bestes & schlechtestes Modell pro Eingabetext ---")
for text_label, data in per_text.items():
    print(f"  {text_label}: Bestes={data['best_model']} ({data['best_score']:.1f})  |  Schlechtestes={data['worst_model']} ({data['worst_score']:.1f})")

print("\n--- Konsistenz über Eingabetexte (Spread = max - min) ---")
